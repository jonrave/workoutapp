...
| `packages/adapters` | Step 4 | Manual entry, guided protocols (CMJ median-of-5, home-BP 7-day), Oura raw-signals-only mapping, raw HR stream ingestion (Strava streams or file drop; device zone labels rejected at the boundary), and free-text parsing behind an interface (drafts require confirmation; the Claude parser is an optional separate export). |
...
```sh
npm install
npm test                      # 133 tests incl. all 37 scenario fixtures
